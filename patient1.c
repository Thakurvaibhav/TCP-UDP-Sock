...
printf("\nPhase 3: Patient 1 has  dynamic UDP port number %u and IP address %s.",(unsigned)ntohs(adr_inet.sin_port),inet_ntoa(*((struct in_addr *)lh->h_addr)));
...
in = fopen("patient1insurance.txt","r");
...
//strcpy(insurance,strtok(NULL,"\n"));
    ...
    printf("\n\nPhase 1: Authentication request from Patient 1 with username %s and password %s has been sent to the Health Center Server", e, f);
	...
	{
	printf("\n\nPhase 1: Patient 1 authentication result: %s . Exiting...",buf);
	close(sockfd);
	return 0;
	}
	printf("\n\nEnd of Phase 1\n");
...
printf("\n Phase 2: The requested appointment is available and reserved to patient 1.\nThe assigned doctor port number is %s",prt);
}
...
printf("\n\n---------------------PHASE 3 BEGINS-------------------------------------------");
    ...
    phasedoc(prt);
    printf("\n\nPhase 3: The cost estimation request request from Patient 1 with insurance plan %s has been sent to the doctor with port number %s and IP address %s",insurance,prt,inet_ntoa(adr_inet.sin_addr));
   	printf("\n\nPhase 3: Patient 1 receives %s $ estimation cost from doctor with port number %s and name %s",cost,prt,doc);
