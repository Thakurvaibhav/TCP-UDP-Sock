...
printf("\n\nPhase 3: Doctor 2 has a static UDP port %s and IP Address %s ", MYPORT,inet_ntoa(adr_inet.sin_addr));
...
strcpy(r_ins,buf);
		printf("\nPhase 3: Doctor 2 receives the request from the patient with port number %d and the insurance plan %s",(int)ntohs(((struct sockaddr_in *)(&their_addr))->sin_port), buf);
...
strcpy(ins1,buf);	
printf("\n\nPhase 3: Doctor 2 has sent estimated price %s to patient with port number %u",ins1,(int)ntohs(((struct sockaddr_in *)(&their_addr))->sin_port));
